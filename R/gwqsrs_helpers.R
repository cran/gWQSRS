...
rs.estimate_param = function(slctd_vars, dtf, Q, b1_pos, b1_constr, family, zilink, zero_infl, formula, ff, weights, control){
...
rs.model.fit_f = function(i, param, slctd_vars, dtf, Q, b1_pos, family, zilink, formula, ff, weights, zero_infl){
...
rs.par.modl.est <- function(dtf, Q, formula, ff, weights, rs, n_vars, b1_pos, b1_constr, family, zilink, zero_infl, plan_strategy, seed, control){

  if (family$family %in% c("gaussian", "quasipoisson")) ts = "t"
  else if (family$family %in% c("binomial", "poisson", "multinomial", "negbin")) ts = "z"

  if(!is.numeric(rs)) stop("'b' must be a number")
  ...
  param = future_lapply(X = slctd_vars, FUN = rs.estimate_param, dtf = dtf, Q = Q, b1_pos = b1_pos,
                        ...
                        formula = formula, ff = ff, weights = weights, control = control, future.seed = FALSE)
  ...
  b_fit = future_lapply(X = 1:rs, FUN = rs.model.fit_f, param = param, slctd_vars = slctd_vars, dtf = dtf, Q = Q,
                        ...
                        weights = weights, zero_infl = zero_infl, future.seed = FALSE)
